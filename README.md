## 简介
一个权限管理系统，实现了完整的用户、组织机构、角色资源、字典、定时任务、文件管理、登录日志等功能。  
一个独立的用户中心，提供单点登录、统一会话及前后端分离跨域的解决方案。  
一个动态实时配置的网关代理，提供了在线配置、限速、日志与流量统计等功能。  
一个在线文档系统，集成语法增强版的Markdown文档、在线画图、数据库设计（ER模型）、在线数据表格，支持团队协作、版本控制与分享。  
