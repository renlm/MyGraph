...
CREATE TABLE gateway_proxy_config(
    ...
    enabled                  	TINYINT(1)      DEFAULT 1  		NOT NULL  	COMMENT '是否启用（默认启用）',
    cros_allowed				TINYINT(1)		DEFAULT 0 		NOT NULL	COMMENT '是否允许跨域（默认否）',
    cros_origin_patterns		VARCHAR(255)	DEFAULT NULL				COMMENT '跨域来源限制（匹配规则，多个逗号分隔）',
